...
try:
    ...
    print("[INFO] Sionna loaded successfully")  # 추가
...
def _create_mitsuba_scene_xml(
    ...
    """
    Human mesh를 포함한 Mitsuba XML scene 파일 생성.
    
    Args:
        mesh_ply_path: Human mesh PLY 파일 경로
        tx_position: Transmitter 위치 [x, y, z]
        rx_position: Receiver 위치 [x, y, z]
        output_dir: 출력 디렉토리
        
    Returns:
        생성된 XML 파일 경로
    """
    xml_path = os.path.join(output_dir, "scene.xml")
    meshes_dir = os.path.join(output_dir, "meshes")
    os.makedirs(meshes_dir, exist_ok=True)
    
    # PLY 파일을 meshes 디렉토리로 복사
    import shutil
    mesh_filename = os.path.basename(mesh_ply_path)
    dest_ply = os.path.join(meshes_dir, mesh_filename)
    if mesh_ply_path != dest_ply:
        shutil.copy(mesh_ply_path, dest_ply)
    
    # Mitsuba XML scene 생성
    # Sionna는 itu-radio-material을 사용
    xml_content = f'''<?xml version="1.0" encoding="utf-8"?>
<scene version="3.0.0">
    <integrator type="path"/>
    
    <!-- Ground plane with concrete material -->
    ...
    <!-- Human mesh -->
    <shape type="ply" id="human_body">
        <string name="filename" value="meshes/{mesh_filename}"/>
        <bsdf type="itu-radio-material" id="mat-human">
            <string name="type" value="concrete"/>
            <!-- Note: Sionna doesn't have built-in human material, 
                using concrete as approximation. 
                You can create custom material if needed -->
        </bsdf>
    </shape>
    
    <!-- Optional: Add walls for indoor scenario -->
    <!-- 
    <shape type="rectangle" id="wall_back">
        <bsdf type="itu-radio-material">
            <string name="type" value="concrete"/>
        </bsdf>
        <transform name="to_world">
            <scale x="10" y="3" z="1"/>
            <translate x="0" y="-5" z="1.5"/>
            <rotate x="1" angle="90"/>
        </transform>
    </shape>
    -->
...
class SionnaRayVisualizer:
    ...
    def __init__(self, config: Optional[RayTracingConfig] = None):
        ...
        self._temp_dir = tempfile.mkdtemp(prefix="sionna_scene_")  # 추가
    ...
    def generate_and_visualize(
        ...
        for i, frame_idx in enumerate(frame_indices):
            ...
            # Run ray tracing
            if SIONNA_AVAILABLE:
                print(f"  [DEBUG] Using SIONNA ray tracing")  # 추가
                paths, coverage = self._run_sionna_ray_tracing_fixed(mesh_data, frame_idx)
            else:
                print(f"  [DEBUG] Using FALLBACK ray tracing (Sionna not available)")
    ...
    def _run_sionna_ray_tracing_fixed(
        self,
        mesh_data: Dict[str, np.ndarray],
        frame_idx: int,
    ) -> Tuple[Dict, Dict]:
        """
        수정된 Sionna ray tracing - human mesh를 scene에 포함.
        
        이 함수를 SionnaRayVisualizer 클래스의 _run_sionna_ray_tracing 메서드로 교체하세요.
        """
        # Sionna 사용 가능 여부 확인
        try:
            import sionna
            from sionna.rt import load_scene, Transmitter, Receiver, PlanarArray
            from sionna.rt import PathSolver  # Sionna 1.x uses PathSolver
        except ImportError:
            return self._run_fallback_ray_tracing(mesh_data, frame_idx)
        
        # 1. Human mesh를 PLY로 export
        ply_path = os.path.join(self._temp_dir, f"human_frame_{frame_idx:04d}.ply")
        self.converter.export_to_ply(mesh_data, ply_path)
        
        # 2. Mitsuba XML scene 생성 (human mesh 포함)
        xml_path = _create_mitsuba_scene_xml(
            mesh_ply_path=ply_path,
            tx_position=self.config.tx_position,
        ...
        try:
            ...
            print(f"[Warning] Failed to load custom scene: {e}")
            print("[Warning] Falling back to default scene...")
        ...
        # 8. PathSolver로 경로 계산 (Sionna 1.x API)
        try:
            # Sionna 1.x uses PathSolver
            ...
            print(f"[Warning] PathSolver failed: {e}")
            # Fallback to old API (Sionna 0.16.x)
            try:
                ...
                print(f"[Warning] compute_paths also failed: {e2}")
                return self._run_fallback_ray_tracing(mesh_data, frame_idx)
        
        # 9. Path 데이터 추출
        path_data = self._extract_path_data(paths)
    ...
    def _extract_path_data(self, paths) -> Dict:
        """
        Sionna paths 객체에서 시각화용 데이터 추출.
        """
        try:
            # paths.vertices: 경로의 각 점들
            # paths.types: 경로 타입 (LOS, reflection, diffraction 등)
            # paths.tau: 지연 시간
            # paths.a: 복소 진폭
            ...
            if isinstance(vertices, np.ndarray):
                print(f"[Debug] Number of paths found: {vertices.shape}")
            else:
                print(f"[Debug] Paths type: {type(vertices)}")
            ...
            print(f"[Warning] Failed to extract path data: {e}")
            return {
                "vertices": [],
                "types": [],
                "delays": np.array([]),
    ...
    def _plot_3d_scene(
        ...
        if isinstance(path_powers, np.ndarray) and path_powers.size > 0:
            # 다차원 배열을 1D로 flatten
            path_powers_flat = path_powers.flatten()
            
            # min/max를 float으로 명시적 변환
            vmin = float(np.nanmin(path_powers_flat))
            vmax = float(np.nanmax(path_powers_flat))
            ...
            cmap = plt.cm.viridis
            has_power_data = True
        elif isinstance(path_powers, list) and len(path_powers) > 0:
            # 리스트인 경우
            vmin = float(min(path_powers))
            vmax = float(max(path_powers))
            if np.isclose(vmin, vmax):
                vmin, vmax = 0.0, 1.0
            norm = Normalize(vmin=vmin, vmax=vmax)
            cmap = plt.cm.viridis
            has_power_data = True
            path_powers_flat = path_powers
        else:
        ...
        for i in range(num_paths_to_plot):
            ...
            if isinstance(path, np.ndarray) and path.ndim == 2:
                if has_power_data and i < len(path_powers_flat):
                    power = float(path_powers_flat[i])
                    color = cmap(norm(power))
                    alpha = 0.3 + 0.7 * norm(power)
                else:
                    color = 'yellow'
                    alpha = 0.5
